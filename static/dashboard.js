(function () {
  ...
  function computeDaysPending(r) {
    if (!r.created_at) return null;
    const start = new Date(r.created_at);
    if (isNaN(start.getTime())) return null;
    // The count freezes as of Installed Date or Canceled Date, whichever
    // applies, rather than continuing to climb with the current date.
    const freezeAt = r.installed_at || r.canceled_at;
    const end = freezeAt ? new Date(freezeAt) : new Date();
  ...
  const DATE_SORT_KEYS = new Set(["created_at", "installed_at"]);
  ...
  function getSortedRequests(list) {
    ...
    return [...list].sort((a, b) => {
      if (sortColumn === "daysPending") {
        const av = computeDaysPending(a);
        const bv = computeDaysPending(b);
        return ((av === null ? -1 : av) - (bv === null ? -1 : bv)) * dir;
      }
      let av = a[sortColumn];
      let bv = b[sortColumn];
      if (DATE_SORT_KEYS.has(sortColumn)) {
  ...
  function updateSortIndicators() {
    document.querySelectorAll(".sort-btn").forEach((btn) => {
      ...
      arrow.style.transform = isActive && sortDirection === "desc" ? "rotate(180deg)" : "rotate(0deg)";
    });
  }
  ...
  function getFilteredRequests() {
    return allRequests.filter((r) => {
      ...
      const searchOk = !searchQuery ||
        ...
        (r.siteUrl || "").toLowerCase().includes(searchQuery);
      return statusOk && marketOk && rdOk && assigneeOk && searchOk;
    });
  }
  ...
  function render() {
    ...
    const marketLabel = selectedMarkets.size > 0 ? ` in ${selectedMarkets.size} Nexstar market${selectedMarkets.size > 1 ? "s" : ""}` : "";
    ...
    const searchLabel = searchQuery ? ` matching "${searchInput.value.trim()}"` : "";
    filterLabel.textContent = `Showing ${statusLabel}${marketLabel}${rdLabel}${assigneeLabel}${searchLabel} (${filtered.length})`;
    ...
    const sorted = getSortedRequests(filtered);
    ...
    for (const r of sorted) {
      ...
      tr.innerHTML = `
        ...
        <td class="cell-dim">${escapeHtml(r.clientContactName) || "—"}</td>
        <td class="cell-dim">${escapeHtml(r.clientContactEmail) || "—"}</td>
        ...
        <td class="cell-dim">${(() => { const d = computeDaysPending(r); return d === null ? "—" : d; })()}</td>
        <td>
  ...
  function downloadCsv() {
    ...
    const headers = ["Client", "Script", "Site URL", "Status", "Assignee", "Client Address", "Client Contact Name", "Client Contact Email", "Nexstar Market", "RD", "Nexstar AE Name", "Nexstar AE Email", "Submitted Date", "Installed Date", "Days Pending Install"];
    const lines = [headers.join(",")];

    const sorted = getSortedRequests(rows);
    for (const r of sorted) {
      const daysPending = computeDaysPending(r);
      lines.push([
        ...
        csvEscape(r.address),
        csvEscape(r.clientContactName),
        csvEscape(r.clientContactEmail),
        csvEscape(r.market),
        ...
        csvEscape(daysPending === null ? "" : daysPending),
      ].join(","));
    }
  ...
  bulkDownloadTemplate.addEventListener("click", () => {
    const csv =
      "Client Name,Site URL,Client Address,Client Contact Name,Client Contact Email,Nexstar Market,Nexstar AE Name,Nexstar AE Email\n" +
      "Riverside Auto Group,https://riversideauto.com,123 Main St Sioux Falls SD,Sam Rivera,sam@riversideauto.com,Sioux Falls,Jane Doe,jane.doe@nexstar.com\n";
  ...
  const CSV_HEADER_MAP = {
    ...
    "nexstar aes name": "contactName",
    "nexstar ae name": "contactName",
    "ae name": "contactName",
    "contact email": "contactEmail",
    "your email": "contactEmail",
    "nexstar aes email": "contactEmail",
    "nexstar ae email": "contactEmail",
    "ae email": "contactEmail",
  };
